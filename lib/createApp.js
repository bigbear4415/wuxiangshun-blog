...
const createApp = () => {
  ...
  const storage =
    process.env.VERCEL || store.isSupabaseEnabled
  ...
  app.get("/api/runtime-config", (_request, response) => {
    response.json({
      ...
      localMusicUploadEnabled: !process.env.VERCEL,
      localGitPushEnabled: !process.env.VERCEL,
  ...
  // ── 文章点赞 ────────────────────────────────────────────
  app.get("/api/posts/:id/likes", async (request, response) => {
    try {
      const count = await store.getPostLikes(request.params.id);
      response.json({ likes: count });
    } catch (error) {
      console.error(error);
      response.status(500).json({ message: "获取点赞数失败。" });
    }
  });

  app.post("/api/posts/:id/like", async (request, response) => {
    try {
      const count = await store.addPostLike(request.params.id);
      if (count === null) {
        response.status(404).json({ message: "文章不存在。" });
        return;
      }
  ...
  // ── 文章评论 ────────────────────────────────────────────
  app.get("/api/posts/:id/comments", async (request, response) => {
    try {
      const comments = await store.getPostComments(request.params.id);
      response.json(comments);
    } catch (error) {
      console.error(error);
      response.status(500).json({ message: "获取评论失败。" });
    }
  });
  ...
  app.post("/api/posts/:id/comments", async (request, response) => {
    ...
    if (!isAnonymous && !authorName) {
      response.status(400).json({ message: "实名评论请填写昵称。" });
      return;
    }

    try {
      const comment = await store.addPostComment(request.params.id, { content, isAnonymous, authorName });
      if (!comment) {
        response.status(404).json({ message: "文章不存在。" });
        return;
      }
      ...
      console.error(error);
      response.status(500).json({ message: "提交评论失败。" });
    }
  });
  ...
  const handleMusicUpload = (request, response, next) => {
    if (process.env.VERCEL) {
      response.status(400).json({ message: "线上环境不能把音乐文件写回你本机的 music 文件夹。请在本地启动博客服务后上传。" });
      return;
    }

    musicUpload.array("music", 10)(request, response, next);
  };

  app.post("/api/music/upload", requireAuth, handleMusicUpload, async (request, response) => {
    try {
      const files = request.files || [];
      if (!files.length) {
        response.status(400).json({ message: "没有选择任何音乐文件。" });
        return;
      }
      ...
      response.json({ message: `成功上传 ${uploaded.length} 个音乐文件。`, files: uploaded });
    } catch (error) {
      console.error(error);
  ...
  app.delete("/api/music/:filename", requireAuth, async (request, response) => {
    if (process.env.VERCEL) {
      response.status(400).json({ message: "线上环境不能删除你本机 music 文件夹里的文件。请在本地启动博客服务后操作。" });
      return;
    }
    ...
    try {
      ...
      // 安全校验：防止路径穿越
      if (!filePath.startsWith(resolvedMusicDir)) {
        response.status(400).json({ message: "非法文件路径。" });
        return;
      }

      await fs.unlink(filePath);
      response.json({ message: `已删除音乐文件: ${filename}` });
    } catch (error) {
      ...
      console.error(error);
      response.status(500).json({ message: "删除音乐文件失败。" });
    }
  });
  ...
  const runGit = (args) =>
    new Promise((resolve, reject) => {
      execFile("git", args, { cwd: rootDir, windowsHide: true }, (error, stdout, stderr) => {
        if (error) {
          reject(new Error((stderr || stdout || error.message || "Git 命令执行失败。").trim()));
          return;
        }
  ...
  app.post("/api/local-tools/git-push", requireAuth, async (request, response) => {
    if (process.env.VERCEL) {
      response.status(400).json({ message: "线上环境不能替你操作本机 Git。请在本地启动博客服务后使用这个按钮。" });
      return;
    }

    const message = String(request.body?.message || "").trim() || `后台一键发布 ${new Date().toLocaleString("zh-CN")}`;

    try {
      const branch = (await runGit(["rev-parse", "--abbrev-ref", "HEAD"])) || "master";
      ...
      response.json({
        message: committed
          ? `已完成提交并推送到 ${branch}，Vercel 会自动开始部署。`
          : `没有新的本地改动，已尝试将 ${branch} 推送到远端。`,
        committed,
        branch
      });
    } catch (error) {
      console.error(error);
      response.status(500).json({ message: error.message || "执行 git push 失败。" });
    }
  });

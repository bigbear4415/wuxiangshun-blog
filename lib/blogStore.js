...
const createStore = ({ rootDir, dataDir, uploadsDir, musicDir, configFilePath, supabaseUrl, supabaseAnonKey, supabaseServiceRoleKey, supabaseStorageBucket, adminPasswordFallback }) => {
  ...
  const postCommentsFilePath = path.join(dataDir, "post-comments.json");
  const postLikesFilePath = path.join(dataDir, "post-likes.json");
  ...
  const uploadStorageJson = async (objectPath, value) => {
    if (!supabase) {
      throw new Error("Supabase 未启用。");
    }

    const payload = Buffer.from(JSON.stringify(value, null, 2), "utf8");
    const { error } = await supabase.storage.from(supabaseStorageBucket).upload(objectPath, payload, {
      upsert: true,
  ...
  const listStorageObjects = async (folderPath) => {
    if (!supabase) {
      return [];
    }
    ...
    while (true) {
      const { data, error } = await supabase.storage.from(supabaseStorageBucket).list(folderPath, {
        limit: pageSize,
        offset,
  ...
  const downloadStorageJson = async (objectPath) => {
    if (!supabase) {
      return null;
    }

    const { data, error } = await supabase.storage.from(supabaseStorageBucket).download(objectPath);
    if (error) {
      throw error;
    }
  ...
  const getPostLikes = async (postId) => {
    if (isSupabaseEnabled) {
      try {
        const files = await listStorageObjects(`meta/post-likes/${postId}`);
        return files.length;
      } catch {
        return 0;
      }
    ...
    try {
      const likes = await readJsonFile(postLikesFilePath);
      return typeof likes[postId] === "number" ? likes[postId] : 0;
    } catch {
      return 0;
    }
  };
  ...
  const addPostLike = async (postId) => {
    ...
    if (isSupabaseEnabled) {
      ...
      const objectPath = `meta/post-likes/${postId}/${Date.now()}-${crypto.randomUUID()}.json`;
      await uploadStorageJson(objectPath, { postId, createdAt });
      return getPostLikes(postId);
    }

    let likes = {};
    try {
      likes = await readJsonFile(postLikesFilePath);
    } catch {
      // start fresh
    }
    likes[postId] = (likes[postId] || 0) + 1;
    await writeJsonFile(postLikesFilePath, likes);
    return likes[postId];
  };
  ...
  const getPostComments = async (postId) => {
    if (isSupabaseEnabled) {
      try {
        const files = await listStorageObjects(`meta/post-comments/${postId}`);
        const comments = await Promise.all(
          files.map((file) => downloadStorageJson(`meta/post-comments/${postId}/${file.name}`))
        );
    ...
    try {
      const all = await readJsonFile(postCommentsFilePath);
      return all
  ...
  const addPostComment = async (postId, payload) => {
    const post = await getPost(postId);
    if (!post) return null;
    ...
    const comment = {
      id: crypto.randomUUID(),
      postId,
      authorName: payload.isAnonymous ? "匿名读者" : String(payload.authorName || "").trim(),
      isAnonymous: Boolean(payload.isAnonymous),
    ...
    if (isSupabaseEnabled) {
      const objectPath = `meta/post-comments/${postId}/${Date.now()}-${comment.id}.json`;
      await uploadStorageJson(objectPath, comment);
      return comment;
    }

    let all = [];
    try {
      all = await readJsonFile(postCommentsFilePath);
    } catch {
      // start fresh
    }
    all.push(comment);
    await writeJsonFile(postCommentsFilePath, all);
    return comment;
  };
